				...
				try:
					if isinstance(condition, dict):
						result = len(frappe.get_list(d, fields=["name"],
							filters=condition, limit_page_length = 999, as_list=True, ignore_ifnull=True))
					else:
	...
	for d in items:
		...
		if filters:
			...
			total = len(frappe.get_all(d, fields='name',
				filters=filters, limit=999, distinct=True, ignore_ifnull=True))
			data['open_count'] = total

		total = len(frappe.get_all(d, fields='name',
			filters={fieldname: name}, limit=999, distinct=True, ignore_ifnull=True))
		data['count'] = total
		out.append(data)
